...
### Exam.10-? MOD ###
# 極力改変せずに引数群の有無と異常なオプションをバリデーションしている
# 初期の設計では「-br」のように排他的オプションを指定した場合、後勝ちでリストア処理のみが行われるため、
# b,rが2回以上指定されている場合help関数を呼ぶよう指定
if test -z "$*" -o -n "`echo $1 | grep -E '(^[^-]|[br]{2,})'`" ; then
    help
fi
### END ###
...
### Exam.10-? MOD ###
# OPTINDが変数として参照されておらず、オプションインデックスのシフト処理が行われていないので修正
shift $(($OPTIND - 1))
### END ###
